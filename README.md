...
| M11 | Text and fonts | done — TTF baked at import to a glyph page and integer metrics, a hand-drawn built-in font so text works with no assets, a `Label` node, layout that is the same on every machine. No shaping for complex scripts |
| M12 | UI | done — `Control`, `Panel`, `Button`, `VBox`/`HBox`, anchors and offsets, layout and hit testing *inside the tick* so a click replays, a screen-space render layer. No focus traversal policy: scripts move focus |
| M13 | Input breadth | done — gamepads behind the `pad` feature, an analogue stick quantised into something a replay can hold, the pointer in the input frame, bindings from `project.toml`. Touch is not built: there is no platform with one yet |
